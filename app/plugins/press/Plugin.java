...
import plugins.press.io.PressFileGlobber;
...
public class Plugin extends PlayPlugin {
    ...
    private static String getSingleCompressedCSSUrl(String requestKey) {
        return getCompressedUrl("controllers.press.Press.getSingleCompressedCSS", requestKey);
    }

    private static String getSingleCompressedJSUrl(String requestKey) {
        return getCompressedUrl("controllers.press.Press.getSingleCompressedJS", requestKey);
    }

    private static String getCompressedCSSUrl(String requestKey) {
        return getCompressedUrl("controllers.press.Press.getCompressedCSS", requestKey);
    }

    private static String getCompressedJSUrl(String requestKey) {
        return getCompressedUrl("controllers.press.Press.getCompressedJS", requestKey);
    }
    ...
    private static String getLinkTag(String src) {
        return "<link href=\"" + src + "\" rel=\"stylesheet\" type=\"text/css\" charset=\"utf-8\">"
                + (plugins.press.PluginConfig.htmlCompatible ? "" : "</link>") + "\n";
    }
}
